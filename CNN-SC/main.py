...
from models.TextSRU import ModelSRU
...
def run(**kwargs):
    ...
    if global_opt.use_model == 'TextCNN':
    ...
    elif global_opt.use_model == 'TextSRU':

        model_opt = models.TextSRU.ModelConfig()  
        ...
        if global_opt.status == 'train':
            models.TextSRU.train_eval(model_opt)

        elif global_opt.status == 'test':
            models.TextSRU.test(model_opt)
